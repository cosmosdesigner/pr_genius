...
export function parsePRUrl(url: string): AzureDevOpsParams | null {
  try {
    ...
    if (pathParts.length >= 6 && pathParts[2] === '_git' && pathParts[4] === 'pullrequest') {
      return {
...
async function fetchWithAuth(url: string, pat: string) {
  ...
  if (!response.ok) {
    if (response.status === 401) throw new Error('Unauthorized: Invalid PAT or insufficient permissions.');
    if (response.status === 403) throw new Error('Forbidden: Your PAT might not have "Code (Read)" scope.');
    const errorData = await response.json().catch(() => ({}));
    throw new Error(`Azure DevOps Error (${response.status}): ${errorData.message || response.statusText}`);
  }
  return response.json();
}

export async function getPRDetails(params: AzureDevOpsParams, pat: string): Promise<PRInfo> {
...
export async function getPRMetadata(params: AzureDevOpsParams, pat: string) {
  ...
  try {
    ...
    if (iterationList.length > 0) {
      ...
      changes = iterChangesData.changes || iterChangesData.changeEntries || iterChangesData.value || [];
    }
  } catch (e) {
  ...
  if (changes.length === 0) {
    ...
    changes = changesData.changeEntries || changesData.value || changesData.changes || [];
  }

  return { commitId, changes };
}

export async function fetchBatchContents(params: AzureDevOpsParams, pat: string, changes: any[], commitId: string | null): Promise<FileDiff[]> {
  return Promise.all(
    changes.map(async (change: any) => {
      const path = change.item?.path || change.path || (change.item && typeof change.item === 'string' ? change.item : null);
      if (!path) return null;
      ...
      if (!skipTypes.includes(changeType.toLowerCase())) {
        try {
          const versionParam = commitId ? `&versionDescriptor.version=${commitId}&versionDescriptor.versionType=commit` : '';
          const url = `https://dev.azure.com/${params.organization}/${params.project}/_apis/git/repositories/${params.repository}/items?path=${encodeURIComponent(path)}${versionParam}&api-version=7.0`;
          
          const res = await fetch(url, { headers: { 'Authorization': `Basic ${btoa(':' + pat)}` } });
          if (res.ok) {
      ...
      return { path, changeType, content, originalPath: change.sourceServerItem || change.originalPath } as FileDiff;
    })
